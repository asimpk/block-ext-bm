...
const AddPersonalNoteDialog = (props: ConfirmationDialogRawProps) => {
    const { handleClose, handleAdd, handleShowFolder, handleMove, handleUpdate, note, exemptFolderId } = props;
    ...
    const handleClick = () => {
        if (folder && title && description) {
            handleAdd(folder, title, description)
        }
    }
    ...
    const filterPersonalNotes = exemptFolderId ? personalNotes.filter(pNote => pNote?.folderId !== exemptFolderId) : personalNotes;
    ...
    return (
        ...
        >
            ...
            <Typography component="h1" variant="h6" sx={{ padding: "10px 30px" }}>
                {exemptFolderId ? "Move" : note ? "Update" : "Add"} Note</Typography>

            {
                filterPersonalNotes?.length > 0 ?
                    <>
                                        ...
                                        exemptFolderId ?
                                            <FormControl variant="standard" margin="normal" required fullWidth>
                                                ...
                                                >
                                                    ...
                                                    {filterPersonalNotes.map((option) => (
                        ...
                        <DialogActions>
                            ...
                            </Button>
                            {
                                exemptFolderId ? <Button variant='outlined' onClick={() => { folder && handleMove(folder) }}>Move</Button>
                                    : note ? <Button variant='outlined' onClick={() => handleUpdate(note.id, title, description, folder)}>Update</Button> :
...
export default AddPersonalNoteDialog